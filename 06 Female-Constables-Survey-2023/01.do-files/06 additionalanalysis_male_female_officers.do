...
use "$psfs_clean_dta\psfs_combined.dta"
keep ps_dist_id strata
tempfile psfs_rough
save `psfs_rough'
...
merge m:1 ps_dist_id using "$FC_survey_clean_dta\femaleconstables_ps_avg_selectvars.dta" //merging with collapsed female constables data
...
esttab model* using "$FC_survey_tables\table_jointsig_MO_Techskills.tex" , replace ///
...
cap file close _all
	file open myfile using "$FC_survey_tables\table_jointsig_MO_Techskills.tex", write append
	...
	file write myfile " \textbf{Technical skills index created using inverse covariance weighting, and normalised for control group mean.}"
	file write myfile "  Source: Male officers' survey."
	file write myfile " $^* p<0.10$, $^{**} p<0.05$, $^{***} p<0.01.$ \\"
	file write myfile "(1) \textless{}GBV incident\textgreater{} Based on your experiences, how likely are you to ask a female police personnel to accompany you? \\"
	file write myfile "(2) \textless{}non-GBV incident\textgreater{} How likely are you to ask a female police personnel to accompany you? \\"
	file write myfile "(3) \textless{}alcohol-related incident\textgreater{} How likely are you to ask a female police personnel to accompany you? \\"
...
cd "$FC_survey_tables\" 	
	// Define file paths
	local original "table_jointsig_MO_Techskills.tex"
...
ren index_psfs_gen_And_bl ind_psfs_gen_And 
ren index_psfs_fem_infra_And_bl ind_psfs_fem_infra_And 
ren index_psfs_m_f_seg_And_bl ind_psfs_m_f_seg_And

collapse (mean) q801r_el_dum q801s_el_dum q801t_el_dum q801m_el_dum ind_Tech_And_bl ind_Tech_And_el ind_Desir_And_bl ind_psfs_gen_And ind_psfs_fem_infra_And ind_psfs_m_f_seg_And, by(ps_dist_id_el) //collapsing to PS-level
...
merge m:1 ps_dist_id using `psfs_rough'
drop if _m != 3
drop _m
...
esttab model* using "$FC_survey_tables\table_ps_commonvars.tex" , replace ///
	b(2) se(2)  label noconstant booktabs ///
	star(* 0.10 ** 0.05 *** 0.01) r2 ///
	noobs ///
	...
	cap file close _all
	file open myfile using "$FC_survey_tables\table_ps_commonvars.tex", write append
	file write myfile "\vspace{-\baselineskip}"
	file write myfile "\begin{flushleft}"
	file write myfile "\begin{footnotesize}"
	file write myfile "\raggedright"
	file write myfile " All regressions are for stations with assigned male and female officers."
	file write myfile " All regressions include district and officer strength (at the station level) fixed effects."
...
cd "$FC_survey_tables\" 	
	// Define file paths
	local original "table_ps_commonvars.tex"
	local modified "modified_mytable_slides.tex"
