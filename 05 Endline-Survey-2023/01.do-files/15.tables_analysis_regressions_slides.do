...
global tables "$MO_endline\03.tables\Slides\"
...
log using "$MO_endline_log_files\analysis_slides_log.log", replace text
...
esttab model1 model2 model3 model4 using "$MO_endline_tables\regression_table_`i'_notransfers_slides.tex" , replace ///
	b(2) se(2)  label noconstant booktabs ///
	star(* 0.10 ** 0.05 *** 0.01) r2 ///
	noobs ///
	keep (treatment_bl) ///
...
cap file close _all
	file open myfile using "$MO_endline_tables\regression_table_`i'_notransfers_slides.tex", write append
	file write myfile "\vspace{-\baselineskip}"
	file write myfile "\begin{flushleft}"
...
foreach i in index_VictimBlame_And index_Techskills_And index_Empathy_And index_Flexibility_And index_AttitudeGBV_And index_ExtPol_And index_Discrimination_And index_Truth_And index_Openness_And index_Combined_And index_Combined_disag_And { 	
	// Define file paths
	local original "regression_table_`i'_notransfers_slides.tex"
	local modified "modified_mytable.tex"
...
`stationcontrols' /// station controls
`officercontrols' if dum_transfer == 0, /// officer controls
absorb(`stratafe') cluster (ps_dist_id_bl) 
sum `i'_el if e(sample) == 1 & treatment_el == 0 & dum_transfer == 0
local control_mean = r(mean)
estadd scalar cgmean = `control_mean'
...
esttab model* using "$MO_endline_tables\regression_table_combined_slides.tex" , replace ///
	b(2) se(2)  label noconstant booktabs ///
	star(* 0.10 ** 0.05 *** 0.01) r2 ///
	noobs ///
	keep (treatment_bl) ///
	title("Treatment effects on indices (never transferred)") ///
	nonotes nonote ///
	mtitles ("Victim-blaming" "Technical" "Empathy" "Flexibility" "Attitudes - GBV" "Externalising" "Discrimination" "Truthfulness") ///
	scalars("cgmean Control mean" "FE Strata FE" "Baseline Baseline controls" "Control Station controls" "Secondary Officer controls" "clusters Number of clusters" "obs Number of officers") ///
	sfmt(2)
	
	
cap file close _all
	file open myfile using "$MO_endline_tables\regression_table_combined_slides.tex", write append
	file write myfile "\vspace{-\baselineskip}"
	file write myfile "\begin{flushleft}"
	file write myfile "\begin{tiny}"
	file write myfile "\raggedright"
	file write myfile "  Source: Baseline and endline surveys, police station facility survey. \\" 
	file write myfile "\hyperlink{VictimBlame}{\beamerreturnbutton{Victim-blaming}} \hyperlink{TechSkills}{\beamerreturnbutton{Legal \& technical skills}} \hyperlink{Empathy}{\beamerreturnbutton{Empathy}} \hyperlink{Flexibility}{\beamerreturnbutton{Flexibility}} \hyperlink{AttitudeGBV}{\beamerreturnbutton{Attitudes towards GBV}} \hyperlink{ExtPol}{\beamerreturnbutton{Externalising police responsibilities regarding GBV}} \hyperlink{Discrimination}{\beamerreturnbutton{Discrimination toward lower caste women}} \hyperlink{Truthfulness}{\beamerreturnbutton{Truthfulness of police complaints}}"
	file write myfile "\end{tiny}"
	file write myfile "\end{flushleft}"
	file close myfile
...
cd "$tables" 	
	// Define file paths
	local original "regression_table_combined_slides.tex"
	local modified "modified_mytable.tex"
...
while r(eof) == 0 {
    ...
    if strpos("`line'", "\begin{table}[htbp]\centering") {
        ...
        file write newfile "\caption{Treatment effects on male officer indices (never transferred)}" _n
