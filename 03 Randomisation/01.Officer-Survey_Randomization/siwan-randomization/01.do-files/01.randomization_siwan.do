/*==============================================================================
File Name:	Randomization: Siwan Baseline-Survey-2022
...
*/ 
	...
	global dropbox "C:\Users\dibbo\Dropbox\Debiasing Police in India\005-Data-and-analysis-2022\03 Randomisation\01.Officer-Survey_Randomization\siwan-randomization"
	}
	
else if "`c(username)'"=="Acer"{
	global dropbox "D:\Dropbox_SB\Dropbox\Debiasing Police in India\005-Data-and-analysis-2022\Randomization-2022\01.Officer-Survey_Randomization\siwan-randomization"
	}
...
log using "${log_files}randomisation_siwan.log", replace text
...
use "$source\1011_siwan-merged.dta"
...
* We have on average 23.84 officers per station. The median is 21 officers. 
* We have 28 stations. 13 PS should fall above the median, 15 should fall below the median.

unique ps_dist_id if above_median_officers == 1
unique ps_dist_id if above_median_officers == 0 
//We have 11 PS with above median number of Police Officers and 17 PS with below median Police Officers strength. 

save "${intermediate_dta}1011_siwan-merged_prep.dta", replace
...
save "${intermediate_dta}randomization_siwan.dta", replace
...
*********************************************
use "${intermediate_dta}1011_siwan-merged_prep.dta", clear

merge m:1 ps_dist_id using "${intermediate_dta}randomization_siwan.dta", gen(mergeT) // Complete match
...
cap n			outreg2  using "${tables}randomization_siwan.xls" , label excel dec(3) drop() append addtext(Mean of control, `meanY', Mean of treatment, `meanYT', Strata FE, Yes, Cluster Station, Yes) ctitle ("`var'") nocons 
...
export excel ps_dist_id ps_name ps_confirm  T using "${clean_dta}treatment-stations_siwan.xls", firstrow(variables) replace
