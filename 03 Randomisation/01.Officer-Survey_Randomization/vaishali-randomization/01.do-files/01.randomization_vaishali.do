/*==============================================================================
File Name:	Randomization: Vaishali Baseline-Survey-2022
...
*/ 
	...
	global dropbox "C:\Users\dibbo\Dropbox\Debiasing Police in India\005-Data-and-analysis-2022\03 Randomisation\01.Officer-Survey_Randomization\vaishali-randomization"
	}
	
else if "`c(username)'"=="Acer"{
	global dropbox "D:\Dropbox_SB\Dropbox\Debiasing Police in India\005-Data-and-analysis-2022\Randomization-2022\01.Officer-Survey_Randomization\vaishali-randomization"
	}
...
log using "${log_files}randomisation_vaishali.log", replace text
...
use "$source\1012_vaishali-merged.dta"
...
* We have on average 28.78 officers per station. The median is 25 officers. 
* We have 27 stations. 13 PS should fall above the median, 14 should fall below the median.

unique ps_dist_id if above_median_officers == 1
unique ps_dist_id if above_median_officers == 0 
//We have 12 PS with above median number of Police Officers and 15 PS with below median Police Officers strength. 

save "${intermediate_dta}1012_vaishali-merged_prep.dta", replace
...
save "${intermediate_dta}randomization_vaishali.dta", replace
...
*********************************************
use "${intermediate_dta}1012_vaishali-merged_prep.dta", clear

merge m:1 ps_dist_id using "${intermediate_dta}randomization_vaishali.dta", gen(mergeT) // Complete match
...
cap n			outreg2  using "${tables}randomization_vaishali.xls" , label excel dec(3) drop() append addtext(Mean of control, `meanY', Mean of treatment, `meanYT', Strata FE, Yes, Cluster Station, Yes) ctitle ("`var'") nocons 
...
export excel ps_dist_id ps_name ps_confirm  T using "${clean_dta}treatment-stations_vaishali.xls", firstrow(variables) replace
