...
*/ 
	...
	global dropbox "C:\Users\dibbo\Dropbox\Debiasing Police in India\005-Data-and-analysis-2022\03 Randomisation\01.Officer-Survey_Randomization\bhojpur-randomization"
	}
	
else if "`c(username)'"=="Acer"{
	global dropbox "D:\Dropbox_SB\Dropbox\Debiasing Police in India\005-Data-and-analysis-2022\Randomization-2022\01.Officer-Survey_Randomization\bhojpur-randomization"
	}
...
log using "${log_files}randomisation_bhojpur.log", replace text
...
//We have 10 PS with above median number of Police Officers and 25 PS with above median Police Officers strength. 
...
*********************************************
use "${intermediate_dta}1003_bhojpur-merged_prep.dta", clear

merge m:1 ps_dist_id using "${intermediate_dta}randomization_bhojpur.dta", gen(mergeT)
...
cap n			outreg2  using "${tables}randomization_bhojpur.xls" , label excel dec(3) drop() append addtext(Mean of control, `meanY', Mean of treatment, `meanYT', Strata FE, Yes, Cluster Station, Yes) ctitle ("`var'") nocons 
...
export excel ps_dist_id ps_name ps_confirm  T using "${clean_dta}treatment-stations_bhojpur.xls", firstrow(variables) replace


/* Checking the missing values for vars ps_phone ps_electricity
global psfs_check ps_phone ps_electricity

foreach var of varlist $psfs_check{
...
cap n			local meanYT : display %4.3f `r(mean)'
cap n			local pvalue : display `r(p)'
cap n			outreg2  using "${tables}check2.xls" , label excel dec(3) drop() append addtext(Mean of control, `meanY', Mean of treatment, `meanYT', Strata FE, Yes, Cluster Station, Yes) ctitle ("`var'") nocons 
