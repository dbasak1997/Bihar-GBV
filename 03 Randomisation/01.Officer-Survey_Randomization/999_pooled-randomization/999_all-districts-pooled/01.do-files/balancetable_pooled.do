/*==============================================================================
File Name:	Pooled Balance Tables for all districts
...
if "`c(username)'"=="Acer"{
	global dropbox "D:\Dropbox_SB\Dropbox\Debiasing Police in India\005-Data-and-analysis-2022\Randomization-2022\01.Officer-Survey_Randomization\999_pooled-randomization\999_all-districts-pooled"
	}  
...
log using "${log_files}balancetable_alldistricts.log", replace text
...
append using "${raw}\1001_bagaha-merged_prep.dta""${raw}\1002_bettiah-merged_prep.dta""${raw}\1003_bhojpur-merged_prep.dta""${raw}\1004_gopalganj-merged_prep.dta""${raw}\1005_motihari-merged_prep.dta""${raw}\1006_muzaffarpur-merged_prep.dta""${raw}\1008_patna-merged_prep.dta""${raw}\1009_saran-merged_prep.dta""${raw}\1010_sitamarhi-merged_prep.dta""${raw}\1011_siwan-merged_prep.dta""${raw}\1012_vaishali-merged_prep.dta"

sort ps_dist ps_dist_id

replace ps_dist_id="1005_53" if ps_dist==1005 & ps_series=="53"



save "${intermediate_dta}pooled-merged_prep.dta", replace


*Using Randomization Data

clear

use "${raw}\randomization_motihari.dta", clear

replace ps_dist_id="1005_53" if ps_dist_id=="1003_53"

append using  "${raw}\randomization_bettiah.dta""${raw}\randomization_bhojpur.dta""${raw}\randomization_gopalganj.dta""${raw}\randomization_muzaffarpur.dta""${raw}\randomization_patna.dta""${raw}\randomization_saran.dta""${raw}\randomization_sitamarhi.dta""${raw}\randomization_siwan.dta""${raw}\randomization_vaishali.dta""${raw}\randomization_nalanda.dta""${raw}\randomization_bagaha.dta"

sort ps_dist_id

save "${intermediate_dta}pooled-randomization.dta", replace



*Now merging the two datasets

use "${intermediate_dta}pooled-merged_prep.dta", clear

merge m:1 ps_dist_id using "${intermediate_dta}pooled-randomization.dta", gen(mergeT) // Complete match
