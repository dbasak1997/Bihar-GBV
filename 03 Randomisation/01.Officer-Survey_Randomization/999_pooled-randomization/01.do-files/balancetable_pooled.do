/*==============================================================================
File Name:	Pooled Balance Tables for Bagaha, Vaishali, Motihari and Siwan
...
if "`c(username)'"=="HP"{
	global dropbox "C:\Users\HP\Dropbox\Debiasing Police in India\005-Data-and-analysis-2022\Randomization-2022\01.Officer-Survey_Randomization\999_pooled-randomization"
	}
	
else if "`c(username)'"=="AG"{
	global dropbox "C:\Users\AG\Dropbox\Debiasing Police in India\005-Data-and-analysis-2022\Randomization-2022\01.Officer-Survey_Randomization\999_pooled-randomization"
	}
	
else if "`c(username)'"=="Acer"{
	global dropbox "D:\Dropbox_SB\Dropbox\Debiasing Police in India\005-Data-and-analysis-2022\Randomization-2022\01.Officer-Survey_Randomization\999_pooled-randomization"
	}  
...
log using "${log_files}balancetable_pooled.log", replace text
...
use "${raw}\1001_bagaha-merged_prep.dta", clear

append using "${raw}\1005_motihari-merged_prep.dta" "${raw}\1011_siwan-merged_prep.dta"  "${raw}\1012_vaishali-merged_prep.dta"

save "${intermediate_dta}pooled-merged_prep.dta", replace


use "${raw}\randomization_bagaha.dta", clear

append using "${raw}\randomization_motihari.dta" "${raw}\randomization_siwan.dta"  "${raw}\randomization_vaishali.dta"


save "${intermediate_dta}pooled-random.dta", replace

*Now merging the two datasets

use "${intermediate_dta}pooled-merged_prep.dta", clear

merge m:1 ps_dist_id using "${intermediate_dta}pooled-random.dta", gen(mergeT) // Complete match
